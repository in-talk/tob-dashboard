...
const GaugeChart = () => {
  ...
  const getStatusBadge = (percentage:number) => {
    if (percentage >= 80) return { label: "Outstanding", color: "bg-green-100 text-green-800"  };
    if (percentage >= 50) return { label: "Excellent", color: "bg-blue-100 text-blue-800" };
    if (percentage >= 30) return { label: "Good", color: "bg-yellow-100 text-yellow-800" };
    if (percentage >= 10) return { label: "Needs Attention", color: "bg-orange-100 text-orange-800" };
  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">Agent XFER Performance</h1>
          <p className="text-gray-600 dark:text-gray-300">Monitor call transfer performance across your agents</p>
        </div>
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-4 text-center">
                  ...
                  <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-3">
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-400">Transferred</div>
                  </div>
                </div>
              </div>
            );
        ...
        <div className="mt-8 bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-100 dark:border-gray-700">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-4">Team Summary</h2>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="text-center">
              <div className="text-2xl font-bold text-gray-900 dark:text-white">
                {(agentData.reduce((sum, agent) => sum + agent.xferPercentage, 0) / agentData.length).toFixed(1)}%
              </div>
              <div className="text-sm text-gray-500 dark:text-gray-400">Average Transfer Rate</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-green-600 dark:text-green-400">
                {Math.min(...agentData.map(agent => agent.xferPercentage)).toFixed(1)}%
              </div>
              <div className="text-sm text-gray-500 dark:text-gray-400">Best Performance</div>
            </div>
            <div className="text-center">
