...
const UPSTREAM_BASE = `${process.env.KEYWORD_API_URL}/age-classifier/positive-negative-patterns`;
  ...
  try {
    if (req.method === "DELETE") {
      ...
      if (response.status === 404) {
        return res.status(404).json({ error: "Positive/negative pattern not found" });
      }
    ...
    console.error("Positive-negative-patterns [id] proxy error:", error);
