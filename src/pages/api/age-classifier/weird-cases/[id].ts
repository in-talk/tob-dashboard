...
const UPSTREAM_BASE = `${process.env.KEYWORD_API_URL}/age-classifier/weird-cases`;
  ...
  try {
    if (req.method === "DELETE") {
      ...
      if (response.status === 404) {
        return res.status(404).json({ error: "Weird case not found" });
      }
    ...
    console.error("weird-cases [id] proxy error:", error);
